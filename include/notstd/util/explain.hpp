...
#include <exception>
#include <ostream>
#include <notstd/config/error.hpp>
...
namespace notstd::util
{
    namespace detail
    {
        ...
        void start_entry(std::ostream &        s,
                         config::system_error &e,
                         std::size_t           level);
    ...
    struct explainer
    {
      ...
      private:
        static void
        print(std::ostream &os, std::string_view s, std::size_t level = 0);

        template < class Exception >
        static void
        print(std::ostream &os, Exception &e, std::size_t level = 0);
    ...
    auto deduce_return_code(std::exception_ptr ep = std::current_exception())
        -> int;
